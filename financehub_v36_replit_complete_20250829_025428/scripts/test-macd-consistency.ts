...
// Import the existing historical MACD service
async function importHistoricalService() {
  try {
    const { historicalMACDService } = await import('../server/services/historical-macd-service');
    return historicalMACDService;
  } catch (error) {
    console.warn('Historical MACD service not found, creating fallback implementation');
    return {
      getHistoricalMACDValues: async (symbol: string, days: number) => [],
      getHistoricalRSIValues: async (symbol: string, days: number) => [],
      getHistoricalBBValues: async (symbol: string, days: number) => [],
      calculateZScore: (value: number, historical: number[]) => null
    };
  }
}
...
async function testMACDConsistency() {
  ...
  const historicalService = await importHistoricalService();
  ...
  for (const symbol of ETF_SYMBOLS) {
    try {
      ...
      const historicalMACDs = await historicalService.getHistoricalMACDValues(symbol, 90);
      console.log(`   Historical MACD records: ${historicalMACDs.length}`);
      
      const historicalRSIs = await historicalService.getHistoricalRSIValues(symbol, 90);
      console.log(`   Historical RSI records:  ${historicalRSIs.length}`);
      
      const historicalBBs = await historicalService.getHistoricalBBValues(symbol, 90);
      ...
      if (record.macd_line && historicalMACDs.length >= 10) {
        ...
        const zScore = historicalService.calculateZScore(currentMACD, historicalMACDs);
...
async function validateZScoreDistribution() {
  ...
  try {
    ...
    for (const symbol of symbols) {
      const historicalMACDs = await historicalService.getHistoricalMACDValues(symbol, 60);
      ...
      if (historicalMACDs.length >= 30) {
        ...
        const zScores = historicalMACDs.map(macd => 
          historicalService.calculateZScore(macd, historicalMACDs)
