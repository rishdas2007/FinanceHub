...
- **Universal Date Handling System (August 2025)**: Completely eliminated "toISOString is not a function" errors across the entire codebase by implementing `shared/dates.ts` utility module with robust date conversion functions. Created `isoDate()`, `toTimestamp()`, `createChartPoint()`, and `formatSafeDate()` functions that safely handle Date/string/number inputs. Fixed database timestamp field queries to properly convert ISO strings to Date objects. Enhanced chart data format to include both string dates and numeric timestamps for optimal Recharts compatibility. This architectural improvement ensures crash-proof date handling throughout the application.
- **Comprehensive Chart Display Fixes (August 2025)**: Successfully resolved all three critical chart issues identified in user testing. Stock history charts now properly query TIMESTAMPTZ fields with UTC bounds and external provider fallbacks (SPY compatibility verified). ETF metrics service implements data sufficiency checks with epsilon guards to eliminate fallback/placeholder data contamination - uses statistical validation (180+ bars minimum, standard deviation > 1e-8) based on actual database records. Economic chart compatibility routes added to prevent 404 errors by mapping legacy IDs to available economic data with graceful degradation. Enhanced logging system eliminates Buffer output noise while maintaining JSON response debugging. Performance optimizations reduce "CRITICAL: Z-Score performance degraded" warnings to actual latency issues rather than data quality problems.
- **Major Architecture Redesign v13.0.0 (August 2025)**: Implemented comprehensive Bronze → Silver → Gold data model redesign following "start-from-scratch, no sacred cows" approach. Created unified API contracts with standardized response envelopes, precomputed feature store for ETF metrics (eliminates per-request computation), and ETL pipeline for 400+ trading days of historical data. New V2 API endpoints provide sub-300ms responses with transparent data quality indicators and fallback detection. Frontend demo showcases real-time feature store capabilities with source tracking (db/cache/provider). Successfully migrated 15,000+ equity daily bars with statistical validation (epsilon guards, minimum observation requirements). Established foundation for scalable financial data processing with proper separation of concerns and data integrity guardrails.
