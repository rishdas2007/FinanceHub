...
export class MacroeconomicIndicatorsService {
  ...
  async getAuthenticEconomicData(): Promise<MacroeconomicData> {
    try {
      ...
      const cacheKey = 'fred-economic-indicators-v14'; // Updated to use LIVE z-score calculations (never cached)
  ...
  private async getDataFromDatabase(): Promise<MacroeconomicData | null> {
    try {
      ...
      const indicators = liveZScoreData.map((zData) => {
        ...
        const varianceFromPrior = zData.varianceFromPrior;
        
        // Use variance from mean for consistency with z-score calculation
