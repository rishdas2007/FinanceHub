...
for indicator_name, info in indicators_data.items():
    # Get the most recent value (current reading)
    current_value = info['recent_values'][0] if info['recent_values'] else None
    prior_value = info['recent_values'][1] if len(info['recent_values']) > 1 else None
    
    # Calculate z-score (using last 12 months of data)
    if len(info['recent_values']) >= 12:
        recent_12_months = info['recent_values'][:12]
        mean_12m = np.mean(recent_12_months)
        std_12m = np.std(recent_12_months)
        z_score = (current_value - mean_12m) / std_12m if std_12m > 0 else 0
    else:
        z_score = 0
    
    # Calculate 3-month annualized rate
    if len(info['recent_values']) >= 3:
        recent_3_months = info['recent_values'][:3]
        if info['unit'] == 'percent':
            # For percentage data, calculate average and annualize
            avg_3m = np.mean(recent_3_months)
            three_month_annualized = (avg_3m / 3) * 12
        else:
            # For other data, calculate growth rate
            if len(recent_3_months) >= 3 and recent_3_months[2] != 0:
                growth_rate = ((recent_3_months[0] / recent_3_months[2]) - 1) * 100
                three_month_annualized = growth_rate * 4  # Annualize quarterly rate
            else:
                three_month_annualized = 0
    else:
        three_month_annualized = 0
    
    # Calculate 12-month YoY change
    if len(info['recent_values']) >= 12:
        if info['recent_values'][11] != 0:
            yoy_change = ((info['recent_values'][0] / info['recent_values'][11]) - 1) * 100
        else:
            yoy_change = 0
    else:
        yoy_change = 0
    
    # Determine next release date based on frequency
    ...
    elif info['frequency'] == 'meeting':
        next_release = datetime(2025, 7, 30)  # Next FOMC meeting
    else:
        next_release = current_date + timedelta(days=7)
    ...
    record = {
        ...
        'Variance vs Forecast': info.get('variance_forecast', None),
        'Prior Reading': prior_value,
        'Variance vs Prior': info.get('variance_prior', None),
        'Z-Score (12M)': round(z_score, 2),
        '3-Month Annualized Rate': round(three_month_annualized, 2),
        '12-Month YoY Change': round(yoy_change, 2),
        'Unit': info['unit']
    }
...
print("Macroeconomic Indicators Dataset - First 10 rows:")
print(df.head(10).to_string(index=False))
print(f"\nTotal indicators: {len(df)}")
print(f"\nDataset columns: {list(df.columns)}")

# Save to CSV first
df.to_csv('macroeconomic_indicators_dataset.csv', index=False)
print("\nDataset saved as 'macroeconomic_indicators_dataset.csv'")
